...
uint8_t button_remap_array[] = {                                                                                                    // Label    Original value (MQB)
        0x0,
        1,                                                                                                                          // Menu     (1)
        2,                                                                                                                          // Right    (2)
        0x0, 0x0, 0x0,
...
// #endif
    ...
    if (b == 0x55 && n > 1) {                                                                                                       // More than 2 bytes, i.e a data frame not a request.
      slave_frame.pop_byte();
      handle_slave_frame();
      slave_frame.reset();
  ...
  if ((millis() - request_buttons_status_timer) >= 20) {
...
// #endif
  ...
  if (slave_comm_timer >= 30000) {
    if (e_message_initialized || ba_message_initialized) {
#if DEBUG_MODE
      debug_serial.println("Timeout of slave RX - rebooting. ");
#endif
      NVIC_SystemReset();
    }
  }
  if (master_comm_timer >= 30000) {
...
void handle_slave_frame(void) {
  if (slave_frame.get_byte(slave_frame.num_bytes()) != check_frame_checksum(slave_frame)) {                                         // Validate checksum
    return;
  }
...
void print_frame(LinFrame frame) {
  if (frame.num_bytes() > 1) {
    for (uint8_t i = 0; i < frame.num_bytes() + 1; i++) {                                                                           // +1 to show checksum byte
